...
const ASSISTANT_WELCOME_MESSAGE =
  'Hello! I am your TutorSphere assistant. How can I help you today? You can ask me to find a tutor or suggest a course.';
const createDefaultAssistantMessages = (): AssistantChatMessage[] => [
  { role: 'bot', text: ASSISTANT_WELCOME_MESSAGE },
];

const ASSISTANT_QUICK_SUGGESTIONS = [
  { label: 'Find tutors', prompt: 'What tutors are available on this platform?' },
  { label: 'Browse courses', prompt: 'What courses are available on this platform?' },
  { label: 'How to book a tutor?', prompt: 'How do I book a tutor on this platform?' },
];
...
export default function App() {
  ...
  const [chatMessages, setChatMessages] = useState<AssistantChatMessage[]>(createDefaultAssistantMessages);
  ...
  const latestAssistantMessageIndex = useMemo(() => {
    for (let index = chatMessages.length - 1; index >= 0; index -= 1) {
      ...
      if (message.role === 'bot' && message.meta !== 'typing') {
        return index;
      }
    }
    return -1;
  ...
  const resetAssistantChatSession = useCallback(() => {
    chatSessionVersionRef.current += 1;
    setChatMessages(createDefaultAssistantMessages());
    setChatInput('');
  ...
  const handleChatSubmit = async (e: React.FormEvent) => {
    ...
    const activeSessionVersion = chatSessionVersionRef.current;
    ...
    try {
      ...
      if (activeSessionVersion !== chatSessionVersionRef.current) {
        return;
      }
        ...
        <div className="fixed bottom-6 right-6 z-50">
          <AnimatePresence>
            {isChatOpen && (
              ...
              >
                {/* Header */}
                <div className="chatbot-widget-header relative px-5 py-4 pr-16 text-white">
                  <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    aria-label="Close TutorSphere Assistant"
                    className="absolute right-4 top-1/2 h-9 w-9 -translate-y-1/2 rounded-lg border border-white/20 bg-white/15 text-white hover:bg-white/25 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/60 flex items-center justify-center transition-all"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar px-4 py-4 space-y-3 bg-gradient-to-b from-slate-50/80 to-white">
                  {chatMessages.map((msg, i) => (
                    <div key={i} className={`flex flex-col gap-1 ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                      <div className={`max-w-[85%] px-4 py-3 text-[13px] leading-relaxed relative group ${msg.role === 'user'
                        ...
                        {msg.meta === 'typing' ? (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="typing-dot" />
                            <span className="typing-dot" />
                            <span className="text-xs font-semibold text-slate-500">Assistant is typing...</span>
                          </div>
                        ) : (
                      ...
                      {msg.role === 'bot' && msg.meta !== 'typing' && i === latestAssistantMessageIndex && (
                        <div className="max-w-[85%] flex flex-wrap gap-2">
                          {ASSISTANT_QUICK_SUGGESTIONS.map((suggestion) => (
                            <button
                              key={`${suggestion.label}-${i}`}
                              type="button"
                              onClick={() => handleQuickSuggestionClick(suggestion.prompt)}
                              className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-semibold text-slate-600 hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-700 transition-colors"
                            >
                              {suggestion.label}
                            </button>
                          ))}
                ...
                <form onSubmit={handleChatSubmit} className="px-4 py-3 border-t border-slate-100 bg-white/95">
                  <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50/80 px-1 py-1 transition-all focus-within:border-indigo-300 focus-within:bg-white focus-within:shadow-[0_0_0_3px_rgba(99,102,241,0.06)]">
                    <input
                      ...
                      placeholder="Ask about tutors, courses, bookings, and platform help..."
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
            ...
            <div>
              ...
              <ul className="space-y-4 text-sm">
                ...
                <li><button onClick={handleOpenChat} className="hover:text-indigo-400 transition-colors">Q&A Support</button></li>
